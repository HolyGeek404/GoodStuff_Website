...
public class GpuFilterService : IProductFilterService
{
    public List<BaseProductModel> Filter(IEnumerable<BaseProductModel> productList, Dictionary<string, List<string>> selectedFilters)
    {
        ...
        var manufacturers = selectedFilters.GetValueOrDefault("Manufacturer")?.ToHashSet(StringComparer.OrdinalIgnoreCase);
        var gpuNames = selectedFilters.GetValueOrDefault("GpuProcessorName")?.ToHashSet(StringComparer.OrdinalIgnoreCase);
