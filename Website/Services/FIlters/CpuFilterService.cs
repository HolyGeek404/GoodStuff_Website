...
{
       public List<BaseProductModel> Filter(IEnumerable<BaseProductModel> productList, Dictionary<string, List<string>> selectedFilters)
    {
        ...
        var unlockedMultipler = selectedFilters.GetValueOrDefault("UnlockedMultipler")?.ToHashSet(StringComparer.OrdinalIgnoreCase);
        ...
        var filtered = cpus.Where(cpu =>
            ...
            (unlockedMultipler == null || unlockedMultipler.Count == 0 || unlockedMultipler.Contains(cpu.UnlockedMultiplayer.ToString())) &&
